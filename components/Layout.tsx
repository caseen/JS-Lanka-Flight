        ...
        <nav className="flex-1 mt-6 px-3 space-y-2 overflow-y-auto">
          {NAV_ITEMS.map((item) => (
            ...
            >
              ...
              {(isSidebarOpen || isMobileMenuOpen) && <span className="font-bold text-sm">{item.label}</span>}
            </button>
          ))}
        </nav>
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
        <header className="bg-white border-b h-16 flex items-center justify-between px-4 lg:px-6 shrink-0 sticky top-0 z-40">
          <div className="flex items-center gap-4">
            ...
            <h2 className="text-lg lg:text-xl font-black text-slate-800 tracking-tight truncate max-w-[180px] sm:max-w-none">
          ...
          <div className="flex items-center gap-3 lg:gap-6">
            <div className="flex items-center gap-2 lg:gap-3">
              <div className="hidden sm:block text-right">
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none">Logged in</p>
                <p className="text-xs font-bold text-slate-700 truncate max-w-[120px] lg:max-w-[150px]">{user.email}</p>
              </div>
              <div className="w-9 h-9 lg:w-10 lg:h-10 rounded-full bg-blue-100 flex items-center justify-center text-blue-700 font-black border-2 border-white shadow-sm shrink-0">
