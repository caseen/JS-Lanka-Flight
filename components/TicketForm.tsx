...
import { Upload, X, Loader2, Calculator, Info, UserPlus, Plane, Plus, AlertTriangle, User, Calendar } from 'lucide-react';
...
const TicketForm: React.FC<TicketFormProps> = ({ 
  ...
  const [currentPassenger, setCurrentPassenger] = useState<Passenger>({ name: '', eTicketNo: '', type: 'ADT' });
  ...
  const addPassenger = () => {
    ...
    setCurrentPassenger({ name: '', eTicketNo: '', type: 'ADT' });
  };
    ...
    <div className="max-w-5xl mx-auto space-y-4 lg:space-y-6 animate-fadeIn pb-12 relative">
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-4 lg:space-y-6">
          <div className="bg-white p-5 lg:p-6 rounded-2xl border shadow-sm space-y-4 lg:space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <div className="space-y-1">
                <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest px-1">Issued Date</label>
                <div className="relative">
                  <Calendar className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none" size={16} />
                  <input type="date" className="w-full border rounded-xl p-3 outline-none focus:ring-2 focus:ring-blue-500 transition-all font-bold text-sm pr-10" value={formData.issuedDate || ''} onChange={e => setFormData({ ...formData, issuedDate: e.target.value })} required />
                </div>
              </div>
            </div>
          ...
          <div className="bg-white p-5 lg:p-6 rounded-2xl border shadow-sm">
             ...
             <div className="space-y-3 mb-6">
                <div className="flex flex-col sm:flex-row gap-3">
                    <input type="text" className="flex-1 border rounded-xl p-3 text-sm font-bold" placeholder="Passenger Name" value={currentPassenger.name} onChange={e => setCurrentPassenger({ ...currentPassenger, name: e.target.value })} />
                    <input type="text" className="flex-1 border rounded-xl p-3 font-mono text-sm font-bold" placeholder="E-Ticket #" value={currentPassenger.eTicketNo} onChange={e => setCurrentPassenger({ ...currentPassenger, eTicketNo: e.target.value })} />
                </div>
                <div className="flex items-center gap-3">
                    <select 
                      className="flex-1 border rounded-xl p-3 bg-white text-sm font-bold focus:ring-2 focus:ring-blue-500 outline-none"
                    ...
                    >
                        ...
                        <option value="INF">Self (INF)</option>
                    </select>
                    <button type="button" onClick={addPassenger} className="bg-blue-600 text-white px-8 py-3 rounded-xl font-black text-xs uppercase tracking-widest hover:bg-blue-700 active:scale-95">Add Passenger</button>
                </div>
             </div>
                  ...
                  <div key={i} className="flex justify-between items-center bg-slate-50 p-3 lg:p-4 rounded-xl border border-slate-100">
                    <div className="min-w-0">
                      <div className="flex items-center gap-2">
                        <div className="text-xs lg:text-sm font-bold text-slate-800 truncate">{p.name}</div>
                        <span className={`text-[8px] font-black px-1.5 py-0.5 rounded ${
                            p.type === 'CHD' ? 'bg-orange-100 text-orange-600' : 
                            p.type === 'INF' ? 'bg-purple-100 text-purple-600' : 
        ...
        <div className="space-y-4 lg:space-y-6">
          <div className="bg-white p-5 lg:p-6 rounded-2xl border shadow-sm space-y-4">
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between p-4 bg-slate-50 rounded-xl border border-slate-100">
                ...
                <button 
                  type="button"
                  onClick={() => setFormData({...formData, isDummy: !formData.isDummy})}
                  className={`relative inline-flex h-5 w-10 items-center rounded-full transition-colors duration-200 focus:outline-none ${formData.isDummy ? 'bg-orange-500 shadow-sm' : 'bg-slate-300'}`}
                >
                  <span 
                    className={`absolute h-4 w-4 bg-white rounded-full transition-all duration-200 ${formData.isDummy ? 'left-[22px]' : 'left-[4px]'}`} 
                  />
                </button>
              </div>
            </div>
          ...
          <button type="submit" className="w-full py-4 lg:py-5 bg-blue-600 text-white rounded-3xl font-black text-base lg:text-lg shadow-2xl shadow-blue-100 hover:bg-blue-700 transition-all active:scale-95">
